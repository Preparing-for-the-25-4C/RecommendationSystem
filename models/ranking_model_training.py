...
class RankingModelTrainer:
    ...
    def build_dataset(self, interactions: List[Dict], user_profile, question_db) -> Tuple[
        ...
        user_profile: 访问 user_profile[user_id]
        question_db:  访问 question_db[qid], 可能含 question["vector"], question["hot_score"], CFscore
        ...
        for uid, recs in user2records.items():
            ...
            for row in recs:
                qid = row["question_id"]
                # 如果有 label 列, 则使用; 若无尝试从 rating>2 推断
